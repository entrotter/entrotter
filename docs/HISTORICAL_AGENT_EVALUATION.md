...
A same-task direct Anvil scripting comparison, whole-process resource controls, genuine target-user evaluations and independent PR approval remain open. The useful question is whether Entrotter makes experiments easier to reproduce and audit, not whether a model can outperform the exact rule it was told to follow.
