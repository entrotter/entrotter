...
def scan(binary, root, files, allow_test_loopback=False):
    ...
    with tempfile.TemporaryDirectory(prefix="entrotter-links-") as directory:
        ...
        command = [
            str(binary), "--config", os.devnull, "--no-progress", "--hidden",
            "--cache=false", "--insecure=false", "--include-fragments=full",
            "--max-concurrency", "4", "--host-concurrency", "2",
            "--host-request-interval", "300ms", "--max-retries", "1",
            "--timeout", "15", "--root-dir", str(root),
            "--format", "json", "--verbose", "--output", str(output),
        ]
        if not allow_test_loopback:
            command.append("--exclude-all-private")
        result = subprocess.run(
            [*command, "--", *files], cwd=root, env=environment,
...
def main():
    ...
    files = sorted(
        name for name in tracked.decode().split("\0")
    ...
    serialized = json.dumps(evidence, indent=2).replace(str(root), "/workspace/repository")
    args.output.write_text(serialized + "\n")
    print(json.dumps({"files": len(files), "passed": not failed,
                      **{key: report[key] for key in
                         ("total", "unique", "successful", "errors", "timeouts", "excludes")}}))
    if failed:
        print(json.dumps(report["error_map"], indent=2).replace(str(root), "/workspace/repository"))
        raise SystemExit(1)
