...
CASE = Path(__file__).resolve().parents[2] / 'scenarios/benchmarks/causal-v1/cases/19000000.json'
...
class ReadOnlyRPC:
    ...
    def call(self, method, params=None):
        ...
        self.identifier += 1
        body = json.dumps({'jsonrpc': '2.0', 'id': self.identifier,
                           'method': method, 'params': params or []}).encode()
        req = Request(self.url, body, {'Content-Type': 'application/json', 'User-Agent': 'Entrotter/0.1.0'})
        try:
...
class OwnedRPC(ReadOnlyRPC):
    methods = ReadOnlyRPC.methods | {'web3_clientVersion', 'eth_getBalance', 'eth_call',
        'anvil_setBalance', 'anvil_impersonateAccount', 'anvil_stopImpersonatingAccount',
        'evm_setNextBlockTimestamp', 'eth_sendTransaction', 'evm_mine', 'eth_getTransactionReceipt'}
...
def fork(source, url):
    ...
    process = subprocess.Popen([binary, '--host', '127.0.0.1', '--port', str(port),
        '--chain-id', '31337', '--no-mining', '--silent', '--accounts', '0',
        '--memory-limit', '67108864', '--fork-url', url, '--fork-block-number',
        str(source['block_number']), '--no-storage-caching'], stdin=subprocess.DEVNULL,
    ...
    try:
        ...
        while True:
            ...
            try:
                if 'anvil' not in rpc.call('web3_clientVersion').lower() or rpc.call('eth_chainId') != hex(31337):
...
def balances(rpc, actor, tokens):
    return {'native_wei': str(int(rpc.call('eth_getBalance', [actor, 'latest']), 16)),
            'tokens_raw': {t['address']: str(uint_call(rpc, t['address'], '0x70a08231' + actor[2:].zfill(64))) for t in tokens}}
...
def run_direct():
    ...
    if (header['hash'].lower() != source['block_hash'].lower() or
            int(header['number'], 16) != source['block_number']):
        raise DirectError('Wrong upstream source block')
    result = {'source': {**source, 'timestamp': int(header['timestamp'], 16)}, 'branches': []}
    for policy in ['execute', 'preflight']:
        ...
        with fork(source, url) as (rpc, timestamp):
            ...
            for index, slot in enumerate(scenario['steps']):
                ...
                transaction = {'from': actor, 'to': action['to'], 'data': action.get('data', '0x'),
                               'value': hex(int(action.get('value_wei', '0'))), 'gas': hex(action['gas'])}
                execute = True
                ...
                if tx_hash is not None:
                    ...
                    status = 'success' if int(receipt['status'], 16) == 1 else 'reverted'
                    used = int(receipt['gasUsed'], 16)
                    gas += used
                    cost += used * int(receipt['effectiveGasPrice'], 16)
                steps.append({'status': status, 'receipt': receipt, 'balances': balances(rpc, actor, tokens)})
            final = balances(rpc, actor, tokens)
            rpc.call('anvil_stopImpersonatingAccount', [actor])
            result['branches'].append({'policy': policy, 'initial': initial, 'steps': steps,
                                      'final': final, 'gas_used': str(gas), 'gas_cost_wei': str(cost)})
    return result
